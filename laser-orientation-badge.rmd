...
-   603 entries (students), 30 total columns (info)

-   Effect of Time spent on Final grade
...
```{r}
d |> 
  filter(FinalGradeCEMS > 70, 90 > FinalGradeCEMS)
```

What happens when you change the cut-off from 70 to something else? Add
a thought (or more):

-   I can focus on students who have over 70 in Final grade after using filter function key. 
...
```{r}
d |> 
  select(student_id, FinalGradeCEMS, TimeSpent) |> 
  arrange(desc(FinalGradeCEMS)) |>
  filter(FinalGradeCEMS > 70 & TimeSpent > 1000)
```
...
```{r}
ggplot(d, aes(x = total_points_earned)) +
...
```{r}
ggplot(d, aes(y = FinalGradeCEMS, x = TimeSpent)) +
  geom_point()
```
